...
organs_original |>
  ...
  summarise(
    total_rows = n(),
    nonmissing_TOTAL_DD = sum(!is.na(`TOTAL Actual DD`)), 
    non_missing_prop = nonmissing_TOTAL_DD/total_rows
  )|>
  arrange(REPORTYEAR)
```
...
```{r}
organs_summary <- organs_original |>
  group_by(REPORTYEAR) |>
  summarise(
    total_rows = n(),
    nonmissing_TOTAL_DD = sum(!is.na(`TOTAL Actual DD`))
...
ggplot(organs_summary, aes(x = REPORTYEAR)) +
  geom_col(aes(y = total_rows), fill = "lightblue", alpha = 0.6) +
  geom_col(aes(y = nonmissing_TOTAL_DD), fill = "darkblue", alpha = 0.8) +
  labs(
    x = "Year",
    y = "Count",
    title = "Total Rows vs. Non-missing TOTAL Actual DD by Year"
  )  
```

Looking at the non_missing_prop column and the bar graph above , it seems that the ratio of  non_missing values to total_rows per year is almost 1 after 2014. Possible reason for this could be that, the survey instead of adding 'NAs' or '0s', did not report countries that did not report records of organ donation for those years.
